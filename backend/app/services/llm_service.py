"""LLM AI Agent — Gemini 2.5 Flash chiến lược (3 vai trò, không làm toán)"""
import os, json
...
async def check_llm() -> Dict:
    ...
    return {
        ...
        "provider": provider,
        "model": "gemini-2.5-flash" if provider=="Gemini" else ("llama-3.1-70b" if provider=="Groq" else "gpt-4o-mini" if provider=="OpenAI" else "mock-llm-v1"),
        "capability": "PCCC synthesis + Vision + What-if advisor (Gemini 2.5 Flash)",
...
async def synthesis_pccc(fire_score: int, firms_count: int, weather: Dict, district: str = "Huyện Chư Prông") -> Dict:
    ...
    # Try new google-genai SDK first (gemini-2.5-flash)
    try:
...
"""
        ...
        return {"source": "Gemini 2.5 Flash", "provider": "Gemini", "status": "LIVE", "model": "gemini-2.5-flash", **data, "evidence": {"fire_score": fire_score, "firms": firms_count, "weather": weather}}
        ...
        try:
            ...
            if api_key:
                ...
                data = json.loads(response.text)
                return {"source": "Gemini 1.5 Flash", "provider": "Gemini", "status": "LIVE", "model": "gemini-2.5-flash", **data}
        except Exception:
            pass
        # Mock fallback JSON chuẩn
        level = "CRITICAL" if fire_score>=80 else "WARNING" if fire_score>=60 else "WATCH"
        return {
            "source": "Gemini 2.5 Flash (mock fallback)",
...
async def verify_fire_image(image_b64: str, gps: Dict) -> Dict:
    ...
    try:
        ...
        if not api_key:
            raise RuntimeError("GEMINI_API_KEY not configured")
        client = genai_new.Client(api_key=api_key)
        # For vision, use gemini-2.5-flash with image
