...
class OfficialFireWarning(Base):
    ...
    # Join key to administrative_units.id (real commune UUID after seed linkage).
    # Declared as FK so Postgres enforces it; SQLite ignores enforcement but the
    # join is verified by tests (see test_calibration.py). Serverless DBs reseed
    # on deploy; persistent Postgres needs a migration for pre-existing rows.
