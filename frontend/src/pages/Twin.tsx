...
export default function Twin(){
  ...
  const run = async()=>{
    setLoading(true)
    try{
      const r=await fetch(`${(import.meta as any).env?.VITE_API_BASE||'http://localhost:8000'}/api/what-if?rain=20&temp=2`).then(x=>x.json()).catch(()=>null)
      setRes(r||{risk_delta:-18, co2_delta:12, status:'DEMO DATA'})
  ...
  return (
    <div className="page">
      ...
      : <div className="result">Result: Risk {res?.risk_delta ?? -22}% · CO₂ +{res?.co2_delta ?? 14}% · {res?.status||'DEMO DATA'} · {new Date().toLocaleTimeString('vi-VN')} — Baseline vs A · 2024→2030</div>}
